...
// Number base
static const int NUMBER_BASE = ceil(log2(sizeof(WORDS) / sizeof(WORDS[0])));

// Number of checksum bits
static const size_t NUMBER_OF_CHECKSUM_BITS = Mnemonic::SEED_SIZE / ceil(log2(NUMBER_BASE));

// Number of words
static const size_t NUMBER_OF_WORDS = (Mnemonic::SEED_SIZE * numeric_limits<uint8_t>::digits + NUMBER_OF_CHECKSUM_BITS) / NUMBER_BASE;

// Checksum digest algorithm
static const char *CHECKSUM_DIGEST_ALGORITHM = "SHA-256";

// Checksum mask
static const uint8_t CHECKSUM_MASK = (1 << NUMBER_OF_CHECKSUM_BITS) - 1;


// Supporting function implementation

// Display passphrase
void Mnemonic::displayPassphrase(const uint8_t seed[SEED_SIZE]) {
	...
	if(!EVP_DigestUpdate(digestContext.get(), seed, SEED_SIZE)) {
	...
	size_t index = 0;
	for(size_t i = 0; i < SEED_SIZE + sizeof(checksum); ++i) {
	
		// Go through all bits in the byte
		for(int j = ((i < SEED_SIZE) ? numeric_limits<uint8_t>::digits : NUMBER_OF_CHECKSUM_BITS) - 1; j >= 0; --j) {
		
			// Get bit
			const int bit = (((i < SEED_SIZE) ? seed[i] : checksum) >> j) & 1;
		
			// Update word indices at index to include the bit in the number base
			wordIndices[index / NUMBER_BASE] |= bit << ((NUMBER_BASE - 1) - (index % NUMBER_BASE));
	...
	// Display message
	cout << "Recovery passphrase: ";
	
	// Go through all word indices
	for(size_t i = 0; i < NUMBER_OF_WORDS; ++i) {
	
		// Display word
		cout << WORDS[wordIndices[i]] << ' ';
	}
	
	cout << endl;
	
	// Securely clear word indices
	explicit_bzero(wordIndices, sizeof(wordIndices));
}
