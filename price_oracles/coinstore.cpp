...
// Constructor
Coinstore::Coinstore(const TorProxy &torProxy) :
...
// Get new price
pair<chrono::time_point<chrono::system_clock>, string> Coinstore::getNewPrice() const {

	// Check if creating request failed
	vector<uint8_t> response;
	const unique_ptr request = createRequest("api.coinstore.com", Common::HTTPS_PORT, "/api/v1/market/trade/MWCUSDT?size=1", response);
	if(!request) {
	
		// Throw exception
		throw runtime_error("Creating Coinstore request failed");
	}
	...
	if(!performRequests() || response.empty()) {
		...
		// Throw exception
		throw runtime_error("Performing Coinstore requests failed");
	}
	...
	if(!json.is_object() || !json["data"].is_array() || !json["data"].get_array().size()) {
	
		// Throw exception
		throw runtime_error("Coinstore response is invalid");
	}
	
	// Check if most recent price is invalid
	const simdjson::dom::element mostRecentPrice = json["data"].at(0);
	if(!mostRecentPrice.is_object() || !mostRecentPrice["ts"].is_int64() || !mostRecentPrice["price"].is_string()) {
	
		// Throw exception
		throw runtime_error("Coinstore most recent price is invalid");
	}
	...
	if(date < chrono::duration_cast<chrono::milliseconds>(chrono::time_point<chrono::system_clock>::min().time_since_epoch()).count() || date > chrono::duration_cast<chrono::milliseconds>(chrono::time_point<chrono::system_clock>::max().time_since_epoch()).count()) {
		...
		// Throw exception
		throw runtime_error("Coinstore date is invalid");
	}
	...
	for(const char *i = price; *i; ++i) {
		...
		if(!isdigit(*i) && *i != '.') {
			...
			// Throw exception
			throw runtime_error("Coinstore price is invalid");
		}
	}
	...
	if(resultSize <= 0) {
		...
		// Throw exception
		throw runtime_error("Getting Coinstore result size failed");
	}
	...
	if(mpfr_sprintf(result.data(), ("%." + to_string(precision) + "R*F").c_str(), MPFR_RNDN, mwcPrice) != resultSize) {
		...
		// Throw exception
		throw runtime_error("Getting Coinstore result failed");
	}
