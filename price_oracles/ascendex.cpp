...
// Constructor
AscendEx::AscendEx(const TorProxy &torProxy) :
...
// Get new price
pair<chrono::time_point<chrono::system_clock>, string> AscendEx::getNewPrice() const {

	// Check if creating request failed
	vector<uint8_t> response;
	const unique_ptr request = createRequest("ascendex.com", Common::HTTPS_PORT, "/api/pro/v1/trades?symbol=MWC/USDT&n=1", response);
	if(!request) {
	
		// Throw exception
		throw runtime_error("Creating AscendEX request failed");
	}
	...
	if(!performRequests() || response.empty()) {
		...
		// Throw exception
		throw runtime_error("Performing AscendEX requests failed");
	}
	...
	if(!json.is_object() || !json["data"].is_object() || !json["data"]["data"].is_array() || !json["data"]["data"].get_array().size()) {
	
		// Throw exception
		throw runtime_error("AscendEX response is invalid");
	}
	
	// Check if most recent price is invalid
	const simdjson::dom::element mostRecentPrice = json["data"]["data"].at(0);
	if(!mostRecentPrice.is_object() || !mostRecentPrice["ts"].is_int64() || !mostRecentPrice["p"].is_string()) {
	
		// Throw exception
		throw runtime_error("AscendEX most recent price is invalid");
	}
	
	// Get date
	const int64_t date = mostRecentPrice["ts"].get_int64().value();
	...
	if(date < chrono::duration_cast<chrono::milliseconds>(chrono::time_point<chrono::system_clock>::min().time_since_epoch()).count() || date > chrono::duration_cast<chrono::milliseconds>(chrono::time_point<chrono::system_clock>::max().time_since_epoch()).count()) {
		...
		// Throw exception
		throw runtime_error("AscendEX date is invalid");
	}
	...
	for(const char *i = price; *i; ++i) {
		...
		if(!isdigit(*i) && *i != '.') {
			...
			// Throw exception
			throw runtime_error("AscendEX price is invalid");
		}
	}
	...
	if(resultSize <= 0) {
		...
		// Throw exception
		throw runtime_error("Getting AscendEX result size failed");
	}
	...
	if(mpfr_sprintf(result.data(), ("%." + to_string(precision) + "R*F").c_str(), MPFR_RNDN, mwcPrice) != resultSize) {
		...
		// Throw exception
		throw runtime_error("Getting AscendEX result failed");
	}
